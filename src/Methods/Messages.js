...
module.exports = function() {
  ...
  return {
    fromRaw: function(raw) {
       ...
       * @returns {Promise/DiscordMessage} Returns a promise and discord message
       */
       ...
       * @returns {Promise/DiscordMessage} Returns a promise and the message deleted
       */

      raw.delete = function(reason = "") {
        return new Promise((res) => {
          request.req("DELETE", `/channels/${raw.channel.id}/messages/${raw.id}`, {reason: reason} , _this.token).then(m => {
            setTimeout(res, 100, res(_this.message_methods().fromRaw(m)));
          }).catch(error => {
            if (error.status === 403) throw new Error("Missing Permissions");
          });
      ...
      raw.edit = function(newmessage) {
        return new Promise((res) => {
          request.req("PATCH", `/channels/${raw.channel.id}/messages/${raw.id}`, {content: newmessage} , _this.token).then(m => {
            setTimeout(res, 100, res(_this.message_methods().fromRaw(m)));
          }).catch(error => {
            if (error.status === 403) throw new Error("Missing Permissions");
          });
       ...
       * @returns {Promise/DiscordMessage} Returns a promise and discord message (pin)
       */
       ...
       * @returns {Promise/DiscordMessage} Returns a promise and discord message (unpinned message)
       */

      raw.unpin = function() {
        return new Promise((res) => {
          request.req("DELETE", `/channels/${raw.channel.id}/pins/${raw.id}`, {} , _this.token).then(m => {
            setTimeout(res, 100, res(_this.message_methods().fromRaw(m)));
          }).catch(error => {
            if (error.status === 403) throw new Error("Missing Permissions");
          });
